...
export const dateIsoToLocal = (date: string, divisor = '-'): string => {
  if (!date) return ''
...
export const dateIsoToBR = (date: string, divisor = '-'): string => {
  if (!date) return ''
  const _date = new Date(date)
  const day = `${_date.getDate()}`.padStart(2, '0')
  const month = `${_date.getMonth() + 1}`.padStart(2, '0')
...
export const dateUTCIsoToLocal = (date: string, divisor = '-'): string => {
  if (!date) return ''
  const _date = new Date(date)
...
export const formatedDateToJsonApi = (date: Date) => {
